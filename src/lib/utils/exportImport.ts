...
function downloadBlob(blob: Blob, filename: string) {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    throw new Error('Export is only supported in the browser.');
  }
...
export async function exportData(): Promise<void> {
  try {
    // Fetch all data from Supabase using allSettled for graceful error handling
    ...
    // Extract data or handle failures
    const tables = ['income_entries', 'daily_data', 'fixed_expenses', 'payment_plans', 'payment_plan_payments', 'app_settings'];
    ...
    // Update last export date in settings (updated_at is auto-handled by trigger)
    await supabase
      .from('app_settings')
      .update({
...
export async function importData(file: File): Promise<void> {
  ...
  const { exportDataSchema } = await import('@/types/validation/settings.validation');
  ...
  try {
    ...
    if (!parseResult.success) {
      const errors = parseResult.error.errors.map((e: { path: (string | number)[]; message: string }) => `${e.path.join('.')}: ${e.message}`).join(', ');
      throw new Error(`Invalid export file format: ${errors}`);
    }
    const imported = parseResult.data;

    // Clear existing data (all data, no user_id filter needed)
    // Using 'gte created_at' with epoch date to delete all records (more semantically clear than neq hack)
    await Promise.all([
      supabase.from('income_entries').delete().gte('created_at', '1970-01-01'),
      supabase.from('daily_data').delete().gte('created_at', '1970-01-01'),
      supabase.from('fixed_expenses').delete().gte('created_at', '1970-01-01'),
      supabase.from('payment_plan_payments').delete().gte('created_at', '1970-01-01'),
      supabase.from('payment_plans').delete().gte('created_at', '1970-01-01'),
    ]);
    ...
    // Update last import date in settings (updated_at is auto-handled by trigger)
    await supabase
      .from('app_settings')
      .update({
...
export async function clearAllData(): Promise<void> {
  // Using 'gte created_at' with epoch date to delete all records
  await Promise.all([
    supabase.from('income_entries').delete().gte('created_at', '1970-01-01'),
    supabase.from('daily_data').delete().gte('created_at', '1970-01-01'),
    supabase.from('fixed_expenses').delete().gte('created_at', '1970-01-01'),
