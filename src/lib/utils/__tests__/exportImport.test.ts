...
describe('exportImport', () => {
  ...
  let mockDeleteGte: ReturnType<typeof vi.fn>; // For delete().gte() chain
  ...
  beforeEach(() => {
    ...
    vi.spyOn(HTMLAnchorElement.prototype, 'click').mockImplementation(mockAnchorClick as any);
    ...
    mockDeleteGte = vi.fn().mockResolvedValue({ error: null });
    mockGte = vi.fn(() => mockDeleteGte); // delete().gte() returns promise
    ...
    mockDelete = vi.fn(() => ({ gte: mockGte })); // For delete().gte()
  ...
  describe('importData - CRITICAL ERROR HANDLING TEST', () => {
    it('should import valid data successfully', async () => {
      const mockFile = createMockFile(
        JSON.stringify({
	      ...
	      const gteDeleteMock = vi.fn().mockResolvedValue({ error: null });
      ...
      const upsertCalledMock = vi.fn().mockResolvedValue({ error: null });
        ...
        const mockEqChain = {
	      ...
	      expect(gteDeleteMock).toHaveBeenCalledTimes(5); // Called for each delete clear
    ...
    it('should handle partial import failures (CRITICAL BUG FIX TEST)', async () => {
      ...
      const gteDeleteMock = vi.fn().mockResolvedValue({ error: null });
	      const mockDeleteResult = { gte: gteDeleteMock };
