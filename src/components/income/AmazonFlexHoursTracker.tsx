...
export function AmazonFlexHoursTracker({ date, allIncomeEntries }: AmazonFlexHoursTrackerProps) {
  const { amazonFlexDailyCapacity, amazonFlexWeeklyCapacity, loadTheme, setAmazonFlexDailyCapacity } = useStore(
    useShallow((state) => ({
  ...
  const hours = useMemo(() => {
    return calculateAmazonFlexHours(allIncomeEntries, date, dailyLimitHours, weeklyLimitHours);
  }, [date, allIncomeEntries, dailyLimitHours, weeklyLimitHours]);

  const handleDailyLimitChange = useCallback(async (hours: number) => {
    try {
