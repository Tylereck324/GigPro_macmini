...
export function GoalList({
  ...
  const renderGoal = ({ goal, currentAmount, isComplete }: GoalProgress) => (
  ...
  const shouldVirtualize = goals.length > 10;

  return (
    <>
      {shouldVirtualize ? (
        <List
          defaultHeight={Math.min(goals.length * 200, 800)}
          rowCount={goals.length}
          rowHeight={200}
          rowProps={{}}
          rowComponent={({ index, style }) => (
            <div style={style}>
              {renderGoal(goals[index])}
            </div>
          )}
        />
      ) : (
        <div className="space-y-4">
          {goals.map((goalProgress) => renderGoal(goalProgress))}
        </div>
      )}
