...
export function DayCell({ date, currentMonth, profit, onFocus, tabIndex = 0 }: DayCellProps) {
  ...
  return (
    <div
      ...
      role="gridcell"
      tabIndex={tabIndex}
      aria-label={`View details for ${format(date, 'MMMM d, yyyy')}. ${profit && profit.profit > 0 ? `Profit: ${formatCurrency(profit.profit)}` : profit && profit.profit < 0 ? `Loss: ${formatCurrency(profit.profit)}` : 'No activity'}`}
      className={clsx(
    ...
    >
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {hasIncome && (
            <div className={clsx(`${PROFIT_INDICATOR_SIZE} rounded-full`, {
