...
ALTER TABLE variable_expenses ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;
...
-- Variable Expenses
CREATE POLICY "Users can manage their own variable expenses" ON variable_expenses
  FOR ALL USING (auth.uid() = user_id);
